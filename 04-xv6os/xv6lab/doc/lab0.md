...
修改 main.c ，將除了列印以外的功能全部註解掉！

於是除了啟動的 entry.S, start.c 之外，就只剩下 輸出的 console.c, printf.c, uart.c 有用到。
...
```
user@DESKTOP-96FRN6B MINGW64 /d/ccc109/co2os/03-xv1
$ make qemu
...
uartputc(int c)
{
  ...
  while(1){
    if(uart_tx_w == uart_tx_r + UART_TX_BUF_SIZE){
      ...
      uart_tx_w += 1;
      uartstart(); // 呼叫 uartstart() 試著把字元 c 送出去！
...
uartstart()
{
  while(1){
    ...
    if((ReadReg(LSR) & LSR_TX_IDLE) == 0){ // 緩衝區滿了
    ...
    int c = uart_tx_buf[uart_tx_r % UART_TX_BUF_SIZE]; // 使用環狀 queue，取出一個字元
    uart_tx_r += 1;
    
    // maybe uartputc() is waiting for space in the buffer.
    wakeup(&uart_tx_r);
    
    WriteReg(THR, c); // 真正把字元 c 用 UART 寫出去！
  }
}
...
```

其中的 UART0 定義在 memlayout.h 當中
