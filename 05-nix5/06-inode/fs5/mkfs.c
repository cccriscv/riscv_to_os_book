...
void fs_format(char *imgFile, int nfiles, char *files[]) {
  ...
  fsfd = fopen(imgFile, "wb+"); // ccc: fsfd = open(argv[1], O_RDWR|O_CREAT|O_TRUNC, 0666);
  if(fsfd < 0){
    perror(imgFile);
    exit(1);
  }
  ...
  for(i = 0; i < nfiles; i++){
    ...
    if((fd = fopen(files[i], "rb+")) == NULL){ // ccc: if((fd = open(argv[i], 0)) < 0){
      perror(files[i]);
      exit(1);
    }
...
int main(int argc, char *argv[]) {
  ...
  fs_format(argv[1], argc-2, &argv[2]);
  exit(0);
}
